...
void findMatches(vector<SiftDescriptor> imageDesc_1, vector<SiftDescriptor> imageDesc_2, CImg<double>* finalImage, double imageSize, vector<randPoints>* randomPoints, int* countNoOfMatches, bool checkFlow){
...
int findBestMatchForImage(vector<SiftDescriptor> queryImage, vector<SiftDescriptor> inputImage, CImg<double>* finalImage, double imageSize, vector<randPoints>* randomPoints)
{
    ...
    findMatches(queryImage, inputImage, finalImage, imageSize, randomPoints, &countNoOfMatches,true);
...
int main(int argc, char **argv)
{
  try {
    ...
    if(part == "part1")
      {
				...
				findMatches(desc1, desc2, &finalImage, input_image.width(), &randomPoints, false);
    ...
    else if()
    {
                ...
                vector<int> countMatches;
                //create a map
                map<int,string> countMap;
                map<int,string>::const_iterator it;
                int length ; //length of args
                int arrayMatches[length];//give length of args
                ...
                countMatches.push_back(count);
                arrayMatches[i] = count; // at index i (for loop)
                countMap[count] =  //name of file; 
                //end the for loop
                int n = sizeof(arrayMatches)/sizeof(arrayMatches[0]);
                sort(arrayMatches, arrayMatches+n, greater<int>());
				finalImage.get_normalize(0, 255).save("final_2.png");
                for(int i = 0; i < length; i++)
                {
                    cout<< arrayMatches[i] << " "<<endl;
                }
                for(it = countMap.end(); it!=countMap.begin(); it--)
                {
                    cout<<"dec order is"<<it->first<<endl;
                    cout<<"dec order is"<<it->second<<endl;
                }
